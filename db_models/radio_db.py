from config import fip_radio, djam_radio, exclude_radios
...
class RadioExternalFunctions:
    ...
    def get_fip_stations(cls):
        ...
        res = request_post(url=fip_radio['tracks_request_url'], json=json, headers=headers)
        ...
        fip_stations.append(fip_radio['id'])
        for excluded_station in exclude_radios:
            if excluded_station in fip_stations:
                fip_stations.remove(excluded_station)

        for fip_station in fip_stations:
            res_list.append({'id': fip_station, 'url': fip_radio['url']})
    ...
    def get_fip_radio_tracks(cls, radio_name, play_date):
        ...
        while from_date.day == day:
            ...
            url = fip_radio['tracks_request_url']
            json = {'query': '{ grid'
    ...
    def get_radio_tracks(cls, radio_name, play_date):
        try:
            if radio_name == djam_radio['id']:
                return cls.get_djam_radio_tracks_selenium(play_date)

            elif fip_radio['id'] in radio_name:
...
class Radio(BaseExtended, RadioExternalFunctions):
    ...
    def update_radios_list(cls):
        ...
        radios.append(djam_radio)

        db_session = cls.create_db_session()
        db_radios = [radio[0] for radio in db_session.query(cls.name).all()]
        db_session.close()
    ...
    def update_radio_tracks(self):
        ...
        db_tracks = [track[0] for track in db_session.query(Track.common_name).all()]
        db_session.close()
        ...
        db_session = self.create_db_session()
        db_import = db_session.query(DBImport).filter(DBImport.import_date == import_date).first()
        db_session.close()
    ...
    def get_latest_import(self):
        db_session = self.create_db_session()
        db_import = db_session.query(DBImport).filter(DBImport.radio_name == self.name).\
